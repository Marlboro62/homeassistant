...
class Media(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Définir le titre si vide et qu'un media_file existe
        if not self.title and self.media_file:
            filename = os.path.splitext(os.path.basename(self.media_file.path))[0]  # Supprime l'extension
            self.title = filename.replace("_", " ").replace("-", " ")[:99]  # Remplace underscores et tirets par espace, tronque à 99

        # Nettoyer uniquement la description
        if self.description:
        ...
        # Arrondir thumbnail_time à un chiffre après la virgule
        if self.thumbnail_time:
            self.thumbnail_time = round(self.thumbnail_time, 1)

        # Définir add_date si vide
        if not self.add_date:
            self.add_date = timezone.now()

        # Générer un friendly_token unique si vide
        ...
        # Définir si listable
        self.listable = self.state == "public" and self.encoding_status == "success" and self.is_reviewed is True

        super().save(*args, **kwargs)

        # Générer thumbnail à partir d'un uploaded_poster
...
class Tag(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # garder lettres, chiffres et espaces seulement
        title = re.sub(r'[^0-9a-zA-Z\s]', '', title)
        # supprimer espaces en début/fin et réduire les espaces multiples à un seul
...
class Playlist(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # Générer friendly_token si vide
        if not self.friendly_token:
            while True:
                friendly_token = helpers.produce_friendly_token()
